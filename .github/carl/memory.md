...
## Project purpose
CSPeek is a defensive Content-Security-Policy (CSP) configuration
auditing tool for authorised targets. It retrieves HTTP response
headers from user-supplied URLs, inspects Content-Security-Policy and
Content-Security-Policy-Report-Only headers, and produces
deterministic, explainable, rule-based risk assessments written to
screen, JSON, CSV, and SQLite.

## Non-goals
- Offensive capability of any kind: exploitation, payload generation,
  bypass guidance, fuzzing, credential attacks, authentication testing,
  evasion, stealth, or rate-limit avoidance.
- Aggressive discovery: unbounded crawling, port scanning, service
...
- Should multiple CSP headers per response be merged (browser
